...
<?= $this->section('title') ?>Transferir Dinheiro<?= $this->endSection() ?>

<?= $this->section('content') ?>
<div class="max-w-md mx-auto bg-white shadow-md rounded-lg p-6 mt-10">
    <h1 class="text-2xl font-semibold text-gray-700 mb-6 text-center">Transferir Dinheiro</h1>

    <?= form_open('/wallet/transfer', ['class' => 'space-y-6']) ?>
        <div>
            <label for="receiver_email" class="block text-sm font-medium text-gray-700">Email do Destinatário</label>
            <div class="mt-1">
            ...
            <?php if (isset($validation) && $validation->hasError('receiver_email')): ?>
                <p class="mt-2 text-sm text-red-600"><?= esc($validation->getError('receiver_email')) ?></p>
            <?php endif; ?>
        </div>

        <div>
            <label for="amount" class="block text-sm font-medium text-gray-700">Valor da Transferência (R$)</label>
